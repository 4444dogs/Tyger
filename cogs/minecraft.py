...
def setup(client) -> commands.Cog:
    minecraft = commands.Cog(
        "Minecraft",
        "Get info about Minecraft servers, minecraft players, and Hypixel players! (Used to be the Revolt bot, Minecrafter.)",
    )
    ...
    async def mcserver(ctx: commands.CommandContext, ip: str):
        try:
            USE_QUERY = False
            ...
            LookupError
            await ctx.reply("Sorry, an error occured. This usually happens when a server does not is exist or is currently offline.")
    @minecraft.command(description="Get info about a hypixel player!")
    async def mchypixel(ctx: commands.CommandContext, player: str):
        try:
            ...
            await ctx.reply("Player not found! (OR an error has occurred)")
    @minecraft.command(description="Get info about any Minecraft player!")
    async def mcplayer(ctx: commands.CommandContext, player: str):
        ...
        try:
            if not playerUUID:
                ...
                await ctx.reply(f"""UUID: {playerUUID}
